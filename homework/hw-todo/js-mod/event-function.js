...
function eventPressСheckbox(paramEventPressСheckbox) {
  const { cardItem, cardBtnConfirm, cardBtnConfirmChecked, cardItemChecked, cardTodoText, cardTodoTextDel } = paramEventPressСheckbox;
...
function eventCrossCardRemoval(paramEventCardItem) {
  event.target.closest(paramEventCardItem).remove();
  // обновляем localStorage
  const todos = getName();
  const cardDel = todos.filter(todo => todo.id === event.target.closest(paramEventCardItem).id);
...
function eventDelAllCards(paramEventCardItem) {
  const cardInputAll = document.querySelectorAll(paramEventCardItem);
